...
export function AppSidebar() {
  ...
  return <Sidebar className={cn("transition-all duration-300", state === "collapsed" ? "w-16" : "w-64")}>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map(item => <SidebarMenuItem key={item.title}>
                  {item.items ? <Collapsible open={openGroups.includes(item.title)} onOpenChange={() => toggleGroup(item.title)}>
                      <CollapsibleTrigger asChild>
                        <SidebarMenuButton className={cn("w-full justify-between", isGroupActive(item.items) && "bg-sidebar-accent text-sidebar-accent-foreground")}>
                          <div className="flex items-center gap-3">
                            <item.icon className="w-4 h-4" />
                            {state !== "collapsed" && <span>{item.title}</span>}
                          </div>
                          {state !== "collapsed" && <ChevronDown className={cn("w-4 h-4 transition-transform", openGroups.includes(item.title) && "rotate-180")} />}
                        </SidebarMenuButton>
                      </CollapsibleTrigger>
                      {state !== "collapsed" && <CollapsibleContent>
                          <SidebarMenuSub>
                    ...
                    </Collapsible> : <SidebarMenuButton asChild isActive={isActive(item.url!)}>
                      <NavLink to={item.url!}>
                        <item.icon className="w-4 h-4" />
                        {state !== "collapsed" && <span>{item.title}</span>}
                      </NavLink>
